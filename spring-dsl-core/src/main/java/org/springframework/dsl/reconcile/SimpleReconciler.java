...
public class SimpleReconciler {
	...
	@Autowired
	public void setLinterFunction(Linter linterFunction) {
	...
	private Mono<Diagnostic> toDiagnostic(Document doc, ReconcileProblem problem) {
		DiagnosticSeverity severity = getDiagnosticSeverity(problem);
		if (severity!=null) {
			try {
				Diagnostic d = new Diagnostic();
				d.setRange(doc.toRange(new DefaultRegion(problem.getOffset(), problem.getLength())));
