...
public abstract class AbstractAntlrCompletioner implements Completioner {
	...
	/**
	 * Recursive through the parser ATN to process all tokens. When successful (out
	 * of tokens) - collect completion suggestions.
	 */
	private void parseAndCollectTokenSuggestions(AssistHolder assistHolder, ATNState parserState, int tokenListIndex) {
		...
		try {
			...
			log.debug(
					assistHolder.indent + "State available transitions: " + transitionsStr(assistHolder, parserState));
	...
	private void handleAtomicTransition(AssistHolder assistHolder, AtomTransition trans, int tokenListIndex) {
		...
		if (nextTokenMatchesTransition) {
			log.debug(assistHolder.indent + "Token " + nextToken + " following transition: "
					+ toString(assistHolder, trans));
			parseAndCollectTokenSuggestions(assistHolder, trans.target, tokenListIndex + 1);
		} else {
			log.debug(assistHolder.indent + "Token " + nextToken + " NOT following transition: "
					+ toString(assistHolder, trans));
		}
	}
	...
	private void handleSetTransition(AssistHolder assistHolder, SetTransition trans, int tokenListIndex) {
		...
		for (int transitionTokenType : trans.label().toList()) {
			...
			if (nextTokenMatchesTransition) {
				log.debug(assistHolder.indent + "Token " + nextToken + " following transition: "
						+ toString(assistHolder, trans) + " to " + transitionTokenType);
				parseAndCollectTokenSuggestions(assistHolder, trans.target, tokenListIndex + 1);
			} else {
				log.debug(assistHolder.indent + "Token " + nextToken + " NOT following transition: "
						+ toString(assistHolder, trans) + " to " + transitionTokenType);
	...
	private void fillParserTransitionLabels(AssistHolder assistHolder, ATNState parserState, Collection<Integer> result,
			Set<TransitionHolder> visitedTransitions) {
		for (Transition trans : parserState.getTransitions()) {
	...
	private void parseSuggestionsAndAddValidOnes(AssistHolder assistHolder, ATNState parserState,
	...
	private boolean isParseableWithAddedToken(AssistHolder assistHolder, ATNState parserState, Token newToken,
			Set<TransitionHolder> visitedTransitions) {
		if (newToken == null) {
			return false;
		}
		for (Transition parserTransition : parserState.getTransitions()) {
			if (parserTransition.isEpsilon()) { // Recurse through any epsilon transitionsStr
				...
				visitedTransitions.add(transWrapper);
				try {
					if (isParseableWithAddedToken(assistHolder, parserTransition.target, newToken,
							visitedTransitions)) {
						return true;
					}
				} finally {
	...
	private String transitionsStr(AssistHolder assistHolder, ATNState state) {
		...
		List<String> transitionStrings = transitionsStream.map(t -> toString(assistHolder, t))
